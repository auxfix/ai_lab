...
class VoiceCompanion:
    ...
    def speak_with_coqui(self, text, output_path=None, play_audio=True):
        ...
        # Add speed (increase for faster speech)
        if self.voice_settings["speed"] != 1.0:
            params["speed"] = min(
                max(self.voice_settings["speed"], 0.5), 3.0
            )  # Limit 0.5x to 3x

        # Add speaker for multi-speaker models
        if self.voice_settings["speaker"] is not None:
            params["speaker"] = self.voice_settings["speaker"]

        # Add language for multilingual models
        if self.voice_settings["language"] is not None:
            params["language"] = self.voice_settings["language"]

        # Add emotion if supported
        if self.voice_settings["emotion"] is not None:
            params["emotion"] = self.voice_settings["emotion"]

        # DISABLE sentence splitting for faster synthesis
        params["split_sentences"] = False

        if output_path is None:
            # Generate audio WITHOUT sentence splitting
            wav = self.tts.tts(**params)
            ...
            if play_audio:
                ...
                self._play_audio_chunked(wav, sample_rate)

            return wav

    def _play_audio_chunked(self, audio_data, sample_rate=22050, chunk_size=1024):
        """Play audio in chunks with progress"""
        stream = self.p.open(
        ...
        for i in range(0, len(audio_data), chunk_size):
            ...
            if i % (chunk_size * 100) == 0:
                percent = (i / len(audio_data)) * 100
                print(f"Playing: {percent:.1f}%", end="\r")
    ...
    def run_conversation(self, use_mic=False, continuous=False):
        ...
        print(f"Hardware: RTX 3090 (24GB VRAM), 128GB RAM")
...
if __name__ == "__main__":
    ...
    companion = VoiceCompanion(
        tts_engine="coqui",
        coqui_model="tts_models/en/vctk/vits",  # Faster, multi-speaker
